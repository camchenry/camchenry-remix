...
const hasCanonicalUrl = (data: unknown): data is { canonicalUrl: string } => {
  return typeof data === "object" && data !== null && "canonicalUrl" in data;
};

/**
 * Finds the canonical URL by searching all of the loader data that matched this route and finding the last one.
 */
export const useCanonicalUrlFromRoutes = () => {
  let canonicalUrl: string | undefined;
  const matches = useMatches();
  for (const match of matches) {
    if (hasCanonicalUrl(match.data)) {
      canonicalUrl = match.data.canonicalUrl;
    }
  }
  return canonicalUrl;
};
...
export default function App() {
  const canonicalUrl = useCanonicalUrlFromRoutes();
  return (
